...
export function zodiacYear(year) {
...
export function lunarMonths (year) {
  var yearData = Lunar.lunarInfo[year - Lunar.MIN_YEAR];

  var leapMonthNum = yearData[0];
  ...
  months = months.slice(0, (leapMonthNum == 0 ? 12 : 13));
...
export function lunarYearMonths  (year) {
  var monthData = lunarMonths(year);
...
export function betweenLunarDays (year, month, day) {
...
export function betweenSolarDays (year, month, day, l_month, l_day) {
...
export function lunarByBetween (year, between) {
  var lunarArray = [], yearMonth = [], t = 0, e = 0, leapMonthNum = 0, m = '';
  ...
  if (between == 0) {
    ...
    m = (leapMonthNum != 0 && t == leapMonthNum + 1)

      ? ('闰'.chineseMonth(t - 1))

      : chineseMonth(((leapMonthNum != 0 && leapMonthNum + 1 < t) ? (t - 1) : t));

  }

  lunarArray.push(year, t, e); //年 月 日

  lunarArray.push(lunarYear(year),

    zodiacYear(year),

    m,

    chineseNumber(e)); //天干地支年 生肖年 月份 日

  lunarArray.push(leapMonthNum); //闰几月
...
export function chineseMonth (month) {
...
export function toLunar (year, month, day) {
  ...
  if (year == Lunar.MIN_YEAR && month <= 2 && day <= 9) {
